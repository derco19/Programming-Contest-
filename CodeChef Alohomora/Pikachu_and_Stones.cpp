...
int main()
{
    ...
    while(t--)
    {
        ll n;
        cin>>n;
        ll a[n+1],pre[n+2]={0},post[n+2]={0},b[n+2]={0},c[n+2]={0};
        for(ll i=1;i<=n;i++){
            cin>>a[i];
        }
        for(ll i=1;i<=n;i++){
        ...
        {
            c[i]=max(c[i+1]+1,a[i]);
            post[i]=post[i+1]+c[i]-a[i];
        }
        ll mi=1e9;
        if(a[1]>c[2])
        {
            mi=min(mi,post[2]+a[1]+1-c[2]);
        }
        if(a[n]>b[n-1])
        {
            mi=min(mi,pre[n-1]+a[n]+1-b[n-1]);
        }
        for(int i=2;i<n-2;i++)
        {
            if(b[i]==c[i+1])
            mi=min(mi,pre[i]+post[i+1]+1);
            else
            mi=min(mi,pre[i]+post[i+1]);
        }
        cout<<mi<<"\n";
