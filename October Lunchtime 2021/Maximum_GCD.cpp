    ...
    while(t--){
        ...
        for(ll i=0;i<n;i++){
            for(ll j=i+1;j<n;j++){
                if(i==0){
                    if(j==1){
                        ll ans=__gcd(v[j+1][n-1],a[i]*a[j]);
                        ma=max(ma,ans);
                    }
                    else{
                        ll ans=__gcd(v[i+1][j-1],__gcd(v[j+1][n-1],a[i]*a[j]));
                        ma=max(ma,ans);
                    }
                }
                else{
                    if(j-i==1){
                        ll ans=__gcd(v[0][i-1],__gcd(v[j+1][n-1],a[i]*a[j]));
                        ma=max(ma,ans);
                    }
                    else{
                        ll ans=__gcd(v[0][i-1],__gcd(v[i+1][j-1],__gcd(v[j+1][n-1],a[i]*a[j])));
                        ma=max(ma,ans);
                    }
                }
            }
