...
int main()
{
    ...
    while(t--)
    {
        ...
        ll ct=0,s1=0; 
        for(ll i=0;i<n;i++){
            auto p=upper_bound(all(trash),i)-trash.begin();
            auto q=lower_bound(all(trash),i)-trash.begin();
            if(q<trash.size() && trash[q]==i)
            continue;
            else{
                ll p1=INT_MAX;
                if(p>0){
                    p1=min(p1,abs(trash[p-1]-i));
                }
                if(p<trash.size()){
                    p1=min(p1,abs(trash[p]-i));
                }
                s1=s1+p1;
            }
        }
